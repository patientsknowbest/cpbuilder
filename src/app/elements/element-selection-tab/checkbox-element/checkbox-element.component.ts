  ...
  openDialog(): void {
    if (!this.elementComponent.isDialogOpen) {
      ...
      const dialogRef = this.dialog.open(CheckboxElementDialogComponent, {
        data: {
          label: this.label, checkBoxValues: this.checkBoxValues, checkBoxIds: this.checkBoxIds
        },
      });
  ...
  generateCheckboxes() {
    ...
    for (let i = 0; i < this.checkBoxValues.length; i++) {
      options += '                <div class="row">\n' + 
                 ...
                 '                        <input class="form-check-input form-control" style="margin: 0; height: 20px;" type="checkbox" name="' + this.checkBoxIds[i] + '" id="' + this.checkBoxIds[i] + '" value="' + this.checkBoxValues[i] + '"/>\n' + 
